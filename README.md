...
3. Given input sentence $s_1$, repeat generating sentences $s_t$ until $LSH(s_t) \in valid\_mask(s_{t-1})$, $t=2,...$ and stop when the max\_new\_tokens is reached. The valid mask is controlled by the LSH signature of $s_{t-1}$.
### Detection
1. Attempt to remove sentence watermark through sentence-level paraphrasing.
2. Detect the sentences to see if $LSH(s_t) \in valid\_mask(s_{t-1})$, $t=2,...$
### Sample usage
